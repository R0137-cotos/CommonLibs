...
public class Contract extends EntityBase {
	 ...
	 * 解約申込日
	 */
	@Schema(description = "解約申込日", required = false)
	@Temporal(TemporalType.DATE)
	 ...
	 * 更新用課金開始日
	 */
	@Schema(description = "更新用課金開始日", required = false)
	@Temporal(TemporalType.DATE)
	...
	@Schema(description = "計上期間明細", required = false)
	private List<AccountingPeriodDetail> accountingPeriodDetailList;

	/**
	 * 契約変更タイミング
	 */
	@Schema(description = "契約変更タイミング", required = false, allowableValues = "自動更新時(\"0\"),契約期間途中(\"1\")")
	...
	@Valid
	@OneToMany(mappedBy = "contract")
	@Schema(description = "次回更新明細情報", required = false)

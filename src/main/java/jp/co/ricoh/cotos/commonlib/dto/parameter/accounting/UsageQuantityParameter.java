...
public class UsageQuantityParameter extends DtoBase {
	 ...
	 * CSVから取得、従量超過分使用量取込パラメータ取得APIで設定
	 */
	@Schema(description = "使用年月", required = true)
	 ...
	 * CSVから取得、従量超過分使用量取込パラメータ取得APIで設定
	 */
	@Schema(description = "超過使用量", required = true, allowableValues = "range[0,99999]")
	 ...
	 * 従量超過分使用量取込パラメータ取得APIで設定
	 */
	@Schema(description = "エラーID", required = false, allowableValues = "range[0,255]")

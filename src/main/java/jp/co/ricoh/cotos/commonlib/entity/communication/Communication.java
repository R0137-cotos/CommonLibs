...
public class Communication extends EntityBase {
	...
	@Schema(description = "対象文書キー<br />コミュニケーションの対象情報を一意に特定するためのキー情報を格納<br />" //
			+ "見積⇒見積ID<br />" //
			+ "契約⇒契約ID<br />" //
			+ "手配⇒手配業務ID", allowableValues = "range[0,255]") //
	...
	@Schema(description = "承認対象種別<br />" //
			...
			+ "タスクフロー⇒非承認", allowableValues = "新規(\"1\"), 情報変更(\"2\"), 契約変更(\"3\"), キャンセル(\"4\"), 解約(\"5\"), 作業完了報告(\"6\"), 非承認(\"7\"), 売上指示(\"8\"), 売上計上(\"9\"), 承認済差戻(\"10\"), 契約更新(\"11\")", example = "1") //
	...
	@Schema(description = "対象文書画面URL<br />Home画面でレコードクリック時の画面遷移先URLを格納<br />" //
			...
			+ "手配⇒対象文書キーで特定される手配業務情報画面のURL", allowableValues = "range[0,1000]") //
	...
	@Schema(description = "依頼者 (作成時不要)<br />" //
			+ "ワークフローの起点となったユーザーのMoM社員IDを設定<br/>※POST時「RJ社員情報マスタ」存在チェック実施", allowableValues = "range[0,255]") //
	...
	@Schema(description = "伝達者<br />" //
			+ "ユーザー識別子としてMoM社員IDを設定<br/>※POST時「RJ社員情報マスタ」存在チェック実施", allowableValues = "range[0,255]") //
	...
	@Schema(description = "対象文書番号<br />" //
			+ "見積⇒見積番号を設定<br />" //
			+ "契約/手配⇒契約番号を設定", allowableValues = "range[0,255]")
	...
	@Schema(description = "対象文書番号枝番<br />" //
			+ "見積⇒見積番号枝番を設定<br />" //
			+ "契約/手配⇒契約番号枝番を設定", allowableValues = "range[0,99]")
	...
	@Schema(description = "商品グループ名<br />" //
			+ "商品グループマスタの商品グループ名を設定", allowableValues = "range[0,255]") //
	...
	@Schema(description = "件名<br />" + "見積⇒見積の案件名を設定<br />" //
			+ "契約⇒契約の案件名を設定<br />" //
			+ "手配⇒手配業務タイプマスタの手配業務タイプ名を設定", allowableValues = "range[0,255]") //
	 ...
	 */
	@Column(nullable = false)
	@Schema(description = "伝達日時 (作成時不要)", readOnly = true)
	 ...
	 * 依頼者氏名
	 */
	@Schema(description = "依頼者氏名 (作成時不要)", allowableValues = "range[0,255]", readOnly = true)
	private String requestOriginName;

	/**
	 * 伝達者
	 */
	@Schema(description = "伝達者 (作成時不要)", allowableValues = "range[0,255]", readOnly = true)
	private String requestFromName;

	/**
	 * 被伝達者
	 */
	@Schema(description = "被伝達者 (作成時不要)", allowableValues = "range[0,255]", readOnly = true)
	private String requestToName;

	/**
	 * 被伝達者候補
	 */
	@Schema(description = "被伝達者候補 (作成時不要)", allowableValues = "range[0,255]", readOnly = true)

...
public class ContractDto extends DtoBase {
	 ...
	 */
	@NotNull
	@Schema(description = "ライフサイクル状態", required = true, allowableValues = "作成中(\"1\"), 作成完了(\"2\"), キャンセル手続き中(\"3\"), 破棄(\"4\"), 予定日待ち(\"5\"), 締結中(\"6\"), 解約手続き中(\"7\"), 解約予定日待ち(\"8\"), 解約(\"9\"), 旧契約(\"10\")", example = "1")
	 ...
	 */
	@NotNull
	@Schema(description = "ワークフロー状態", required = true, allowableValues = "作成中(\"1\"), 承認依頼中(\"2\"), 承認済(\"3\"), 業務依頼中(\"4\"), 業務処理完了(\"5\"), キャンセル申請中(\"6\"), 売上可能(\"7\"), 解約申請中(\"8\")", example = "1")
	...
	@Min(0)
	@Max(9)
	@Schema(description = "売上計上フラグ", required = true, allowableValues = "range[0,9]")
	 ...
	 * S&S作業依頼作成状態
	 */
	@Schema(description = "S&S作業依頼作成状態", required = false, allowableValues = "未作成(\"0\"),作成済み(\"1\"),作成エラー(\"2\")", example = "1")
	...
	@Valid
	@OneToMany(mappedBy = "contract")
	@Schema(description = "契約チェック結果", required = false)
	...
	@Valid
	@OneToMany(mappedBy = "contract")
	@Schema(description = "契約承認ルート", required = false)
	...
	@Valid
	@OneToMany(mappedBy = "contract")
	@Schema(description = "契約添付ファイル", required = false)
	...
	@Valid
	@OneToMany(mappedBy = "contract")
	@Schema(description = "契約添付ファイル履歴", required = false)
	...
	@Valid
	@OneToMany(mappedBy = "contract")
	@Schema(description = "契約追加編集者社員", required = false)
	...
	@Valid
	@OneToOne(mappedBy = "contract")
	@Schema(description = "契約保守担当CE社員", required = false)
	...
	@Valid
	@OneToOne(mappedBy = "contract")
	@Schema(description = "契約保守担当SS組織", required = false)
	...
	@Valid
	@OneToMany(mappedBy = "contract")
	@Schema(description = "見積明細管理", required = false)
	...
	@Size(max = 255)
	@Column
	@Schema(description = "届先名", required = false, allowableValues = "range[0,255]")
	 ...
	 */
	@Column
	@Schema(description = "解約確定日", required = false)
	 ...
	 */
	@Column
	@Schema(description = "契約情報確定日", required = false)
	...
	@Valid
	@OneToMany(mappedBy = "contract")
	@Schema(description = "契約機種", required = false)
	...
	@Valid
	@OneToOne(mappedBy = "contract")
	@Schema(description = "契約受付担当SS組織", required = false)
	...
	@Valid
	@OneToOne(mappedBy = "contract")
	@Schema(description = "契約導入担当SS組織", required = false)
	...
	@Valid
	@OneToOne(mappedBy = "contract")
	@Schema(description = "契約導入担当CE社員", required = false)
	...
	@Valid
	@OneToOne(mappedBy = "contract")
	@Schema(description = "契約受付担当CE社員", required = false)
	 ...
	 */
	@Size(max = 255)
	@Schema(description = "違約金用FFM発注問合せ番号", required = false, allowableValues = "range[0,255]")
	...
	@Valid
	@OneToOne(mappedBy = "contract")
	@Schema(description = "配送先", required = true)
	...
	@Valid
	@OneToOne(mappedBy = "contract")
	@Schema(description = "配送先SS組織", required = true)
	 ...
	 */
	@Size(max = 1333)
	@Schema(description = "ベンダー向けコメント", required = false, allowableValues = "range[0,1333]")
	 ...
	 * MVBアカウント登録区分
	 */
	@Schema(description = "MVBアカウント登録区分", required = false, allowableValues = "新規登録(\"1\"), 既存使用(\"2\")")
	...
	@Max(9)
	@Min(0)
	@Schema(description = "S&S作業依頼フラグ", required = false, allowableValues = "range[0,9]")
	private Integer ssWorkRequestCreateFlg;

	/**
	 * 統合契約連携用CSV作成状態
	 */
	@Schema(description = "統合契約連携用CSV作成状態", required = false, allowableValues = "未作成(\"0\"), 作成済み(\"1\"),作成エラー(\"2\")")
	private AbsConCsvCreateStatus absConCsvCreateStatus;

	/**
	 * 統合契約連携用CSV作成日
	 */
	@Schema(description = "統合契約連携用CSV作成日", required = false)
	@Temporal(TemporalType.DATE)
	 ...
	 * ARCS期間売保守元契約処理状態
	 */
	@Schema(description = "ARCS期間売保守元契約処理状態", required = false, allowableValues = "未作成(\"0\"),CSV作成済み(\"1\"),対象外(\"2\"),管理対象外(\"3\")")
	...
	@Schema(description = "ARCS期間売保守元契約連携日", required = false)
	private Date arcsPeriodSaleMntOriginLinkAt;

	/**
	 * IFS連携用解約CSV作成状態
	 */
	@Schema(description = "IFS連携用解約CSV作成状態", required = false, allowableValues = "未作成(\"0\"), 作成済み(\"1\"), 作成対象外(\"2\"), 作成エラー(\"3\")")
	private IfsLinkageCsvCreateStatus ifsLinkageCancelCsvStatus;

	/**
	 * IFS連携用解約CSV作成日
	 */
	@Schema(description = "IFS連携用解約CSV作成日", required = false)
	@Temporal(TemporalType.DATE)
	...
	@Max(9)
	@Min(0)
	@Schema(description = "次回契約更新可能フラグ", required = false, allowableValues = "range[0,9]")
	...
	@Max(9)
	@Min(0)
	@Schema(description = "次回自動更新フラグ", required = false, allowableValues = "range[0,9]")
	...
	@Max(9)
	@Min(0)
	@Schema(description = "次回継続可能機種なしフラグ", required = false, allowableValues = "range[0,9]")
	private Integer nextEquipmentNotContFlg;

	/**
	 * 基本契約区分
	 */
	@Schema(description = "基本契約区分", required = false, allowableValues = "基本契約(\"1\"),基本契約_一部(\"2\")")
	 ...
	 */
	@Min(0)
	@Schema(description = "基本契約ID", required = false, allowableValues = "range[0,9223372036854775807]")
	private Long basicContractId;

	/**
	 * 納入日
	 */
	@Schema(description = "納入日", required = false)
	@Temporal(TemporalType.DATE)
	 ...
	 * サービス終了最大延長日
	 */
	@Schema(description = "サービス終了最大延長日", required = false)
	@Temporal(TemporalType.DATE)
	 ...
	 * 延長可能契約月数
	 */
	@Schema(description = "延長可能契約月数", required = false)
	private Long maxExtensionMonths;

	/**
	 * 契約変更タイミング
	 */
	@Schema(description = "契約変更タイミング", required = false)
	...
	@Valid
	@OneToOne(mappedBy = "contract")
	@Schema(description = "回収先", required = true)
	private CollectLocationDto collectLocation;

	/**
	 * RPA連携用CPQ更新CSV作成状態
	 */
	@Schema(description = "RPA連携用CPQ更新CSV作成状態", required = false, allowableValues = "連携済み(\"1\"),対象外(\"2\")")
	private RpaLinkageCpqUpdateCsvCreateStatus rpaLinkageCpqUpdateCsvCreateStatus;

	/**
	 * RPA連携用CPQ更新CSV作成日
	 */
	@Schema(description = "RPA連携用CPQ更新CSV作成日時", required = false)
	@Temporal(TemporalType.DATE)
	...
	@Max(9)
	@Min(0)
	@Schema(description = "機器情報取込不要フラグ", required = false, allowableValues = "range[0,9]")
	...
	@Max(9)
	@Min(0)
	@Schema(description = "振替情報取込不要フラグ", required = false, allowableValues = "range[0,9]")
	...
	@Valid
	@OneToMany(mappedBy = "contract")
	@Schema(description = "契約機種付加情報", required = false)
	...
	@Valid
	@OneToMany(mappedBy = "contract")
	@Schema(description = "契約機種状態管理", required = false)
	 ...
	 */
	@Size(max = 255)
	@Schema(description = "電子契約正式帳票名", required = false, allowableValues = "range[0,255]")
	...
	@Max(9)
	@Min(0)
	@Schema(description = "電子契約帳票EIMアップロードフラグ", required = false, allowableValues = "range[0,9]")
	private Integer electronicContractReportEimUploadFlg;

	/**
	 * 電子契約締結日
	 */
	@Schema(description = "電子契約締結日", required = false)
	@Temporal(TemporalType.DATE)

...
public class ContractChangeSpanMaster extends EntityBaseMaster {
	...
	@Schema(description = "契約変更期間管理マスタID(作成時不要)", allowableValues = "range[0,9999999999999999999]")
	private long id;

	/**
	 * 対象ドメイン
	 */
	@Schema(description = "対象ドメイン", requiredMode = Schema.RequiredMode.NOT_REQUIRED, allowableValues = "共通(\"0\"), 見積(\"1\"), 契約(\"2\"), 手配(\"3\"), 電力_見積(\"101\"), 電力_契約(\"102\")")
	 ...
	 * ライフサイクル状態
	 */
	@Schema(description = "ライフサイクル状態", requiredMode = Schema.RequiredMode.NOT_REQUIRED, allowableValues = "作成中(\"1\"), 作成完了(\"2\"), キャンセル手続き中(\"3\"), 破棄(\"4\"), 予定日待ち(\"5\"), 締結中(\"6\"), 解約手続き中(\"7\"), 解約予定日待ち(\"8\"), 解約(\"9\"), 旧契約(\"10\")")
	private LifecycleStatus lifecycleStatus;

	/**
	 * ワークフロー状態
	 */
	@Schema(description = "ワークフロー状態", requiredMode = Schema.RequiredMode.NOT_REQUIRED, allowableValues = "作成中(\"1\"), 承認依頼中(\"2\"), 承認済(\"3\"), 業務依頼中(\"4\"), 業務処理完了(\"5\"), キャンセル申請中(\"6\"), 売上可能(\"7\"), 解約申請中(\"8\")")
	...
	@Schema(description = "チェックパターン区分", requiredMode = Schema.RequiredMode.REQUIRED, allowableValues = "期間固定(\"1\"), 期間可変(\"2\")")
	...
	@JsonIgnore
	@Schema(description = "契約変更期間日付計算パターンマスタ 開始", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
	private DateCalcPatternMaster fromContractChangeDateCalcPatternMaster;

	/**
	 * 契約変更期間チェック対象日区分 開始
	 */
	@Schema(description = "契約変更期間チェック対象日区分 開始", requiredMode = Schema.RequiredMode.NOT_REQUIRED, allowableValues = "契約承認依頼日(\"1\"), サービス利用希望日(\"2\"), 解約希望日(\"3\"), システム日付(\"4\")")
	...
	@JsonIgnore
	@Schema(description = "契約変更期間日付計算パターンマスタ 終了", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
	private DateCalcPatternMaster toContractChangeDateCalcPatternMaster;

	/**
	 * 契約変更期間チェック対象日区分 終了
	 */
	@Schema(description = "契約変更期間チェック対象日区分 終了", requiredMode = Schema.RequiredMode.NOT_REQUIRED, allowableValues = "契約承認依頼日(\"1\"), サービス利用希望日(\"2\"), 解約希望日(\"3\"), システム日付(\"4\")")
	 ...
	 */
	@Size(max = 255)
	@Schema(description = "チェック対象トランザクションテーブル名", requiredMode = Schema.RequiredMode.NOT_REQUIRED, allowableValues = "range[0,255]")
	 ...
	 */
	@Size(max = 255)
	@Schema(description = "エラーキー", requiredMode = Schema.RequiredMode.NOT_REQUIRED, allowableValues = "range[0,255]")
	 ...
	 */
	@Size(max = 255)
	@Schema(description = "エラーメッセージ置換文字列", requiredMode = Schema.RequiredMode.NOT_REQUIRED, allowableValues = "range[0,255]")
	 ...
	 */
	@Size(max = 255)
	@Schema(description = "エラーフィールド", requiredMode = Schema.RequiredMode.NOT_REQUIRED, allowableValues = "range[0,255]")
	 ...
	 */
	@Min(0)
	@Schema(description = "承認アラート管理マスタID", requiredMode = Schema.RequiredMode.NOT_REQUIRED, allowableValues = "range[0,9223372036854775807]")

...
public class ArrangementWorkOrderMaster extends EntityBaseMaster {
	...
	@Schema(description = "手配業務完了順マスタID", required = true, allowableValues = "range[0,9999999999999999999]")
	private long id;
	...
	@Schema(description = "先実行手配業務タイプマスタID配列", required = true)
	@Lob
	 ...
	 */
	@Column(nullable = false)
	@Schema(description = "チェックタイミング区分", required = true, allowableValues = "業務受付時(\"0\"), 業務完了時(\"1\")")
	 ...
	 */
	@Column(nullable = false)
	@Schema(description = "契約種別種別", required = false, allowableValues = "新規(\"1\"), 契約更新(\"2\"), 再契約(\"3\"), アップグレード(\"4\"), メニュー変更(\"5\"), 基本プラン減数(\"6\"), プラン_オプション_減数_削除(\"7\"), オプション_追加_増数(\"8\"), オプション_追加_減数(\"9\"), 自動更新(\"10\"), 支払周期変更(\"26\")")

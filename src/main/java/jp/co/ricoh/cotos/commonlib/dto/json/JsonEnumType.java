...
public class JsonEnumType {
	...
	public enum EstimationTypeDetails {
		...
		新規("1"), 契約更新("2"), 再契約("3"), アップグレード("4"), メニュー変更("5"), 基本プラン減数("6"), プラン_オプション_減数_削除("7"), オプション_追加_増数("8"), オプション_追加_減数("9"), 自動更新("10"), ライセンス_増数_減数("11"), オンサイトのみ("15");
	...
	public enum ContractTypeDetails {
		...
		新規("1"), 契約更新("2"), 再契約("3"), アップグレード("4"), メニュー変更("5"), 基本プラン減数("6"), プラン_オプション_減数_削除("7"), オプション_追加_増数("8"), オプション_追加_減数("9"), 自動更新("10"), 転入("12"), ライセンス増数("13"), ライセンス減数("14"), オンサイトのみ("15");
	...
	public enum OnOffFlg {

		OFF("0"), ON("1");

		private final String text;

		private OnOffFlg(final String text) {
			this.text = text;
		}
		...
		@JsonCreator
		public static OnOffFlg fromString(String string) {
			return Arrays.stream(values()).filter(v -> v.text.equals(string)).findFirst().orElseThrow(() -> new IllegalArgumentException(String.valueOf(string)));
		}
	}
}

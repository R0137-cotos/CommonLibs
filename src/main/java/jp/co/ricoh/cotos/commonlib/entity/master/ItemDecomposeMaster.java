...
public class ItemDecomposeMaster extends EntityBaseMaster {
	...
	@Schema(description = "ID(作成時不要)", allowableValues = "range[0,9999999999999999999]")
	private long id;
	...
	@JsonIgnore
	@Schema(description = "分解後品種マスタ", requiredMode = Schema.RequiredMode.REQUIRED)
	 ...
	 * 分解後品種区分
	 */
	@Schema(description = "分解後品種区分", requiredMode = Schema.RequiredMode.NOT_REQUIRED, allowableValues = "なし(\"0\"), 基本(\"1\"), オプション(\"2\")")
	private ItemType itemType;

	/**
	 * 分解後費用種別
	 */
	@Schema(description = "分解後費用種別", requiredMode = Schema.RequiredMode.NOT_REQUIRED, allowableValues = "初期費(\"1\"), 月額_定額(\"2\"), 年額(\"3\"), 月額_従量(\"4\"), 違約金(\"5\")")

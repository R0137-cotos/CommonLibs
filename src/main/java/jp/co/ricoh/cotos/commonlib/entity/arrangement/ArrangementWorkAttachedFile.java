...
public class ArrangementWorkAttachedFile extends EntityBase {
	...
	@Schema(description = "手配業務添付ファイルID (作成時不要)", allowableValues = "range[0,9223372036854775807]", readOnly = true)
	private long id;
	 ...
	 */
	@Column(nullable = false)
	@Schema(description = "添付者MoM社員ID(作成時不要)<br/>※POST時「RJ社員情報マスタ」存在チェック実施", allowableValues = "range[0,255]", readOnly = true)
	 ...
	 */
	@Column(nullable = false)
	@Schema(description = "添付者氏名(作成時不要)", allowableValues = "range[0,255]", readOnly = true)
	private String attachedEmpName;

	/**
	 * 添付者組織名
	 */
	@Schema(description = "添付者組織名(作成時不要)", allowableValues = "range[0,255]", readOnly = true)

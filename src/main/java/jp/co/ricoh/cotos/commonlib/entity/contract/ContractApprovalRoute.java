...
public class ContractApprovalRoute extends EntityBase {
	...
	@Schema(description = "契約承認ルートID(作成時不要)", required = true, allowableValues = "range[0,9223372036854775807]", readOnly = true)
	private long id;
	...
	@Schema(description = "契約承認実績(作成時不要)", required = false, readOnly = true)
	 ...
	 */
	@OneToMany(mappedBy = "contractApprovalRoute")
	@Schema(description = "契約承認ルートノード(作成時不要)", required = true, readOnly = true)

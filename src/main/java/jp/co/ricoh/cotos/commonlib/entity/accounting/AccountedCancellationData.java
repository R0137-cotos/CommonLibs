...
public class AccountedCancellationData extends EntityBase {
	 ...
	 * 費用種別
	 */
	@Schema(description = "費用種別", required = false, allowableValues = "初期費(\"1\"), 月額_定額(\"2\"), 年額(\"3\"), 月額_従量(\"4\")")
	 ...
	 * 受注区分
	 */
	@Schema(description = "受注区分", required = false, allowableValues = "訪販(\"1\"), WEB(\"2\")")
	 ...
	 */
	@Temporal(TemporalType.DATE)
	@Schema(description = "キャンセル処理日", required = false)
	private Date canceledDate;

	/**
	 * V-UP連携済フラグ
	 */
	@Schema(description = "V-UP連携済フラグ", required = false, allowableValues = "range[0,9]")
	private Integer vupLinkedFlg;

	/**
	 * 計上キャンセルステータス
	 */
	@Schema(description = "計上キャンセルステータス", required = false, allowableValues = "未連携(\"0\"), 連携準備中(\"1\"), 連携済(\"2\")")

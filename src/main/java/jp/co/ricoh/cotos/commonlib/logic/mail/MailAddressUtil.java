...
public class MailAddressUtil {
	...
	public MailInfoDto getMailInfo(HashMap<ServiceCategory, Long> targetIdMap, long mailMasterId) {
		...
		// メールマスタを取得
		MailMaster mailMaster = mailMasterRepository.findById(mailMasterId).orElse(null);
		// メールマスタが存在しない場合
		if (null == mailMaster) {
		...
		mailMaster.getMailAddressMasterList().stream().forEach(mailAddressMaster -> {//
			...
			case 見積:
				...
				// 見積を取得
				Estimation estimation = estimationRepository.findById(targetIdMap.get(ServiceCategory.見積)).orElse(null);
				// メールアドレスを取得
			...
			case 契約:
				...
				// 契約を取得
				Contract contract = contractRepository.findById(targetIdMap.get(ServiceCategory.契約)).orElse(null);
				// メールアドレスを取得
			...
			case 手配:
				...
				// 手配を取得
				ArrangementWork arrangementWork = arrangementWorkRepository.findById(targetIdMap.get(ServiceCategory.手配)).orElse(null);
				// メールアドレスを取得
			...
			case ライセンス:
				...
				// ライセンス情報を取得
				LicenseInfo licenseInfo = licenseInfoRepository.findById(targetIdMap.get(ServiceCategory.ライセンス)).orElse(null);
				// メールアドレスを取得

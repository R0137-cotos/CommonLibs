...
public class ContractListener {
	...
	public void prePersist(Contract contract) {
		...
		this.setEimLinkedStatus(contract);
	}
	...
	private void appendsContractNumber(Contract contract) {
		...
		if (mvTJmci101MasterRepository == null || checkUtil == null || dbUtil == null) {
			// Beanの取得
			ApplicationContext context = ApplicationContextProvider.getApplicationContext();
			if (mvTJmci101MasterRepository == null)
	...
	private void setEimLinkedStatus(Contract contract) {
		/**
		 * EIM連携済状態
		 */
		if (null == contract.getEimLinkedStatus()) {
			contract.setEimLinkedStatus(EimLinkedStatus.未連携);
		}
	}
}

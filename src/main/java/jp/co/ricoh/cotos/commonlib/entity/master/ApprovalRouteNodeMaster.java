...
public class ApprovalRouteNodeMaster extends EntityBaseMaster {
	...
	@Schema(description = "承認ルートノードマスタID", required = true, allowableValues = "range[0,9999999999999999999]")
	private long id;
	 ...
	 */
	@Column(nullable = false)
	@Schema(description = "承認者種別", required = true, allowableValues = "メイン承認者(\"1\"), 代理承認者(\"2\")", example = "1")
	 ...
	 */
	@Column(nullable = false)
	@Schema(description = "承認者導出方式区分", required = true, allowableValues = "直属上司指定(\"1\"), 組織絶対階層指定(\"2\"), 組織直接指定(\"3\"), ユーザー直接指定(\"4\"), 自己承認(\"5\"), 受付担当CE指定(\"6\"), グループ承認(\"7\")", example = "1")
	private ApproverDeriveMethodDiv approverDeriveMethodDiv;

	/**
	 * 組織階層レベル
	 */
	@Schema(description = "組織階層レベル", required = false, allowableValues = "range[0,9]")

...
public class Estimation extends EntityBase {
	...
	@Schema(description = "見積ID(作成時不要)", allowableValues = "range[0,9223372036854775807]", readOnly = true)
	private long id;
	 ...
	 */
	@Column(nullable = false)
	@Schema(description = "ライフサイクル状態(作成時不要)", allowableValues = "作成中(\"1\"), 作成完了(\"2\"), 受注(\"3\"), 失注(\"4\"), 破棄(\"5\")", example = "1", readOnly = true)
	 ...
	 */
	@Column(nullable = false)
	@Schema(description = "ワークフロー状態(作成時不要)", allowableValues = "作成中(\"1\"), 業務依頼中(\"2\"), 業務処理完了(\"3\"), 承認依頼中(\"4\"), 承認済(\"5\"), 顧客提示済(\"6\")", example = "1", readOnly = true)
	 ...
	 */
	@Column(nullable = false)
	@Schema(description = "見積番号(作成時不要)", allowableValues = "range[0,255]", readOnly = true)
	 ...
	 */
	@Column(nullable = false)
	@Schema(description = "見積番号枝番(作成時不要)", allowableValues = "range[0,99]", readOnly = true)
	 ...
	 */
	@Column(nullable = false)
	@Schema(description = "見積種別(作成時不要)", allowableValues = "新規(\"1\"), 契約変更(\"2\")", example = "1", readOnly = true)
	 ...
	 */
	@Size(max = 255)
	@Schema(description = "見積ワークID(作成時不要)", allowableValues = "range[0,255]", readOnly = true)
	...
	@Max(9)
	@Min(0)
	@Schema(description = "V-UP連携フラグ(作成時不要)", allowableValues = "range[0,9]", readOnly = true)
	...
	@Schema(description = "見積承認ルート(作成時不要)", readOnly = true)
	...
	@Schema(description = "見積操作履歴(作成時不要)", readOnly = true)
	...
	@Schema(description = "見積チェック結果(作成時不要)", readOnly = true)
	 ...
	 */
	@OneToMany(mappedBy = "estimation")
	@Schema(description = "商品（見積用）(作成時不要)", readOnly = true)
	...
	@Valid
	@OneToMany(mappedBy = "estimation")
	@Schema(description = "違約金明細(見積用)", readOnly = true)

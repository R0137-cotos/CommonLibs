...
public class EntityBase {
	...
	@Schema(description = "登録日時(作成時不要)", readOnly = true)
	private Date createdAt;

	@Schema(description = "登録者(作成時不要)", allowableValues = "range[0,255]", readOnly = true)
	private String createdUserId;

	@JsonSerialize(using = UnixTimestampDateSerializer.class)
	@Schema(description = "更新日時(作成時不要)", readOnly = true)
	private Date updatedAt;

	@Schema(description = "更新者(作成時不要)", allowableValues = "range[0,255]", readOnly = true)
	private String updatedUserId;

	@Version
	@Schema(description = "version(作成時不要)", allowableValues = "range[0,9223372036854775807]", readOnly = true)

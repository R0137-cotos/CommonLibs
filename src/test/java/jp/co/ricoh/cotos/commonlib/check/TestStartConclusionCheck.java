...
public class TestStartConclusionCheck {
	...
	public void エラーが発生しないこと() {
		...
		contract.setWorkflowStatus(WorkflowStatus.売上可能);

		checkUtil.startConclusionCheck(contract);
	}
	...
	public void エラーが発生すること_契約種別() throws Exception {
		...
		try {
			...
			Assert.assertEquals(messageInfo.get(0).getErrorMessage(), "契約種別が「新規」「契約変更」以外の契約で締結開始指示は行えません。");
		}
	}
	...
	public void エラーが発生すること_ライフサイクル() {
		...
		try {
			...
			Assert.assertEquals(1, messageInfo.size());
			Assert.assertEquals(messageInfo.get(0).getErrorId(), "RCO00003");
			Assert.assertEquals(messageInfo.get(0).getErrorMessage(), "ライフサイクル状態が「作成完了」以外の契約で締結開始指示は行えません。");
		}
	}
	...
	public void エラーが発生すること_ワークフロー() {
		...
		try {
			...
			Assert.assertEquals(1, messageInfo.size());
			Assert.assertEquals(messageInfo.get(0).getErrorId(), "RCO00003");
			Assert.assertEquals(messageInfo.get(0).getErrorMessage(), "ワークフロー状態が「売上可能」以外の契約で締結開始指示は行えません。");
		}
	}
}

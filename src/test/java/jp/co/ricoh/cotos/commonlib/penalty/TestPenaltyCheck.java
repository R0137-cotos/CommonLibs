...
public class TestPenaltyCheck {
	...
	public void 異常系_違約金情報取得_契約ID未設定() throws Exception {
		...
		try {
			penaltyUtil.getPenaltyInfo(null, new Date(), false);
			fail("エラーなし");
		} catch (ErrorCheckException e) {
			List<ErrorInfo> errorList = e.getErrorInfoList();
			Assert.assertEquals(1, errorList.size());
	...
	public void 異常系_違約金情報取得_契約情報が取得できない() throws Exception {
		...
		try {
			penaltyUtil.getPenaltyInfo(12345L, new Date(), false);
			fail("エラーなし");
		} catch (ErrorCheckException e) {
			List<ErrorInfo> errorList = e.getErrorInfoList();
			Assert.assertEquals(1, errorList.size());
	...
	public void 異常系_違約金情報取得_解約予定日未設定() throws Exception {
		...
		try {
			penaltyUtil.getPenaltyInfo(20L, null, false);
			fail("エラーなし");
		} catch (ErrorCheckException e) {
			List<ErrorInfo> errorList = e.getErrorInfoList();
			Assert.assertEquals(1, errorList.size());
	...
	public void 正常_違約金情報取得_契約_違約金情報なし() throws Exception {
		...
		List<PenaltyInfoDto> resultList = penaltyUtil.getPenaltyInfo(contractId, cancelScheduledDate, false);
		Assert.assertEquals("違約金が発生しない場合、空のリストが返却されること", 0, resultList.size());
	}
	...
	public void 正常_違約金情報取得_契約_違約金情報あり_全解約() throws Exception {
		...
		Long contractId = 20L;
		Date cancelScheduledDate = dateCalcPatternUtil.stringToDateConverter("2020/12/31", "yyyy/MM/dd");
		List<PenaltyInfoDto> resultList = penaltyUtil.getPenaltyInfo(contractId, cancelScheduledDate, true);
	...
	public void 異常系_違約金情報取得_契約_違約金情報あり_部分解約_契約に紐づく見積情報なし() throws Exception {
		...
		Date cancelScheduledDate = dateCalcPatternUtil.stringToDateConverter("2020/12/31", "yyyy/MM/dd");
		try {
			penaltyUtil.getPenaltyInfo(contractId, cancelScheduledDate, false);
			fail("エラーなし");
		} catch (ErrorCheckException e) {
			List<ErrorInfo> errorList = e.getErrorInfoList();
			Assert.assertEquals(1, errorList.size());
	...
	public void 異常系_違約金情報作成() throws Exception {
		...
		Method method = PenaltyUtil.class.getDeclaredMethod("createPenaltyInfoList", Map.class, Date.class, EntityBase.class);
		method.setAccessible(true);
	...
	public void 違約金情報作成_契約() throws Exception {
		...
		resultList.stream().forEach(penaltyInfoDto -> {
			if(20L == penaltyInfoDto.getPenaltyItemMasterId()) {
				...
				Assert.assertEquals("数量が正しく設定されていること", 2, penaltyInfoDto.getQuantity());
				Assert.assertEquals("違約金額が正しく設定されていること", BigDecimal.valueOf(200), penaltyInfoDto.getPenaltyAmountSummary());
				Assert.assertEquals("違約金発生最終解約日が正しく設定されていること", "20201231 23:59:59", dateCalcPatternUtil.dateToStringConverter(penaltyInfoDto.getPenaltyOccurCacnlLastDate(), "yyyyMMdd HH:mm:ss"));
	...
	public void 違約金発生解約日の最終日取得() throws Exception {
		...
		Date serviceTermStart = new Date();
		Date result = (Date)method.invoke(penaltyUtil, item, serviceTermStart);
		Assert.assertEquals("違約金起算日区分が未設定の場合、nullが返却されること", null, result);

		// 違約金起算日区分：サービス開始日 最低契約月数：12
		Long itemMasterId = 16126L;
		serviceTermStart = dateCalcPatternUtil.stringToDateConverter("20200115", null);
		item = itemMasterRepository.findOne(itemMasterId);
		result = (Date)method.invoke(penaltyUtil, item, serviceTermStart);
		Assert.assertEquals("12か月後(指定月含む)-1の月の最終日が設定されていること", "20201130 23:59:59", dateCalcPatternUtil.dateToStringConverter(result, "yyyyMMdd HH:mm:ss"));
		...
		item = itemMasterRepository.findOne(itemMasterId);
		result = (Date)method.invoke(penaltyUtil, item, serviceTermStart);
		Assert.assertEquals("12か月後(指定月含む)の月の最終日が設定されていること", "20201231 23:59:59", dateCalcPatternUtil.dateToStringConverter(result, "yyyyMMdd HH:mm:ss"));
	}
}

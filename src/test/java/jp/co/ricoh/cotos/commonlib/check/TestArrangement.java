...
public class TestArrangement {
	...
	public void ArrangementWorkのテスト() throws Exception {
		...
		// 異常系（@Max ：holdingFlg, businessAcceptBranchNumber）
		BeanUtils.copyProperties(testTarget, entity);
		testTarget.setHoldingFlg(INT_10);
		testTarget.setBusinessAcceptBranchNumber(INT_100);
		result = testSecurityController.callParameterCheck(testTarget, headersProperties, localServerPort);
		Assert.assertTrue(result.getErrorInfoList().size() == 2);
		Assert.assertTrue(testTool.errorIdMatchesAll(result.getErrorInfoList(), ParameterErrorIds.ROT00015));
		Assert.assertTrue(testTool.errorMessageMatchesOne(result.getErrorInfoList(), "保留フラグは最大値（9）を超えています。"));

		// 異常系（@Min ：holdingFlg, businessAcceptBranchNumber）
		BeanUtils.copyProperties(testTarget, entity);
		testTarget.setHoldingFlg(INT_MINUS_1);
		testTarget.setBusinessAcceptBranchNumber(INT_MINUS_1);
		result = testSecurityController.callParameterCheck(testTarget, headersProperties, localServerPort);
		Assert.assertTrue(result.getErrorInfoList().size() == 2);
		Assert.assertTrue(testTool.errorIdMatchesAll(result.getErrorInfoList(), ParameterErrorIds.ROT00027));
	...
	public void ArrangementWorkErrorLogのテスト() throws Exception {
		...
		// 異常系（@Size(max) ：）
		BeanUtils.copyProperties(testTarget, entity);
		testTarget.setErrorMessage(STR_4001);
		testTarget.setApiRequest(STR_4001);
		result = testSecurityController.callParameterCheck(testTarget, headersProperties, localServerPort);
		Assert.assertTrue(result.getErrorInfoList().size() == 2);
		Assert.assertTrue(testTool.errorIdMatchesAll(result.getErrorInfoList(), ParameterErrorIds.ROT00014));
		Assert.assertTrue(testTool.errorMessageMatchesOne(result.getErrorInfoList(), "エラー内容は最大文字数（4000）を超えています。"));

	}

	@Test
	public void ArrangementWorkAttachedFileLinkageのテスト() throws Exception {
		ArrangementWorkAttachedFileLinkage entity = arrangementWorkAttachedFileLinkageRepository.findById(401L).get();
		ArrangementWorkAttachedFileLinkage testTarget = new ArrangementWorkAttachedFileLinkage();
		BeanUtils.copyProperties(testTarget, entity);
